...
def superPrint(priority, title, message):
    ...
    if opt_gui == 'gnome':
        subprocess.call(['zenity', '--width=' + str(opt_gui_width),
                        '--' + priority, '--title=' + title, '--text=' + message])
    elif opt_gui == 'kde':
        # Adapt to kdialog
        ...
        if priority == 'warning':
            ...
            priority = 'msgbox'
        # Print message
        subprocess.call(['kdialog', '--geometry=' + str(opt_gui_width) + 'x' +
                        str(opt_gui_height), '--title=' + title, '--' + priority + '=' + message])
    else:
    ...
    if opt_language_suffix in ('on', 'auto'):
        for language in opt_languages:
            for l in list(language.split(',')):
                ...
                if len(l) == 3:
                    lngList.append(opt_language_suffix_separator + l[0:2])

    for ext in extList:
        for lng in lngList:
            subPath = path.rsplit('.', 1)[0] + lng + '.' + ext
            if os.path.isfile(subPath) is True:
                superPrint("info", "Subtitles already downloaded!",
...
def hashFile(path):
    ...
    try:
        ...
        if filesize < 65536 * 2:
            superPrint("error", "File size error!",
...
def selectionKde(subtitlesResultList):
    ...
    if subtitlesMatchedByName == 0:
        tilestr = ' --title="Subtitles for ' + videoTitle + '"'
        menustr = ' --menu="<b>Video title:</b> ' + videoTitle + \
            '<br><b>File name:</b> ' + videoFileName + '" '
    elif subtitlesMatchedByHash == 0:
        tilestr = ' --title="Subtitles for ' + videoFileName + '"'
        menustr = ' --menu="Search results using file name, NOT video detection. <b>May be unreliable...</b><br><b>File name:</b> ' + videoFileName + '" '
    else:  # a mix of the two
        tilestr = ' --title="Subtitles for ' + videoTitle + '" '
        menustr = ' --menu="Search results using file name AND video detection.<br><b>Video title:</b> ' + \
            videoTitle + '<br><b>File name:</b> ' + videoFileName + '" '

    # Spawn kdialog "radiolist"
    process_subtitlesSelection = subprocess.Popen('kdialog --geometry=' + str(opt_gui_width) + 'x' + str(
        opt_gui_height) + tilestr + menustr + subtitlesItems, shell=True, stdout=subprocess.PIPE)
...
def selectionCLI(subtitlesResultList):
    ...
    for item in subtitlesResultList['data']:
        ...
        if opt_selection_count == 'on':
            subtitlesItem += '> "SubDownloadsCnt: ' + \
                item['SubDownloadsCnt'] + '" '

        if item['MatchedBy'] == 'moviehash':
            print("\033[92m[" + str(subtitlesIndex) +
                  "]\033[0m " + subtitlesItem)
        else:
            print("\033[93m[" + str(subtitlesIndex) +
                  "]\033[0m " + subtitlesItem)
    ...
    while (sub_selection < 0 or sub_selection > subtitlesIndex):
        try:
            sub_selection = int(
                input(">> Enter your choice (0-" + str(subtitlesIndex) + "): "))
...
def selectionAuto(subtitlesResultList):
    ...
    videoFileParts = videoFileName.replace(
    ...
    for idx, subtitle in enumerate(subtitlesResultList['data']):
        ...
        subFileParts = subtitle['SubFileName'].replace(
            '-', '.').replace(' ', '.').replace('_', '.').lower().split('.')
...
def dependencyChecker():
    ...
    if opt_gui != 'cli':
        for tool in ['gunzip', 'wget']:
            ...
            if path is None:
                superPrint("error", "Missing dependency!", "The <b>'" +
                           tool + "'</b> tool is not available, please install it!")
                return False
...
parser.add_argument(
    '-g', '--gui', help="Select the GUI you want from: auto, kde, gnome, cli (default: auto)")
...
parser.add_argument(
    '-i', '--skip', help="Skip search if an existing subtitles file is detected", action='store_true')
parser.add_argument(
    '-s', '--search', help="Search mode: hash, filename, hash_then_filename, hash_and_filename (default: hash_then_filename)")
parser.add_argument(
    '-t', '--select', help="Selection mode: manual, default, auto")
parser.add_argument(
    '-a', '--auto', help="Force automatic selection and download of the best subtitles found", action='store_true')
parser.add_argument(
    '-o', '--output', help="Override subtitles download path, instead of next their video file")
parser.add_argument(
    '-x', '--suffix', help="Force language code file suffix", action='store_true')
parser.add_argument(
    '-8', '--utf8', help="Force UTF-8 file download", action='store_true')
parser.add_argument('-u', '--username',
                    help="Set opensubtitles.org account username")
parser.add_argument('-p', '--password',
                    help="Set opensubtitles.org account password")
parser.add_argument(
    'searchPathList', help="The video file(s) or folder(s) for which subtitles should be searched and downloaded", nargs='+')
...
if opt_gui == 'auto':
    ...
    ps = str(subprocess.Popen(
        ['ps', 'cax'], stdout=subprocess.PIPE).communicate()[0]).split('\n')
    for line in ps:
...
for videoPathDispatch in videoPathList:
    ...
    # Pass settings
    command = [sys.executable, scriptPath, "-g", opt_gui,
               "-s", opt_search_mode, "-t", opt_selection_mode]
...
try:
    ...
    try:
        session = osd_server.LogIn(osd_username, hashlib.md5(osd_password[0:32].encode(
            'utf-8')).hexdigest(), osd_language, 'opensubtitles-download 5.0')
    except Exception:
        # Retry once after a delay (could just be a momentary overloaded server?)
        time.sleep(3)
        try:
            session = osd_server.LogIn(
                osd_username, osd_password, osd_language, 'opensubtitles-download 5.0')
        except Exception:
    ...
    for currentLanguage in opt_languages:
        ...
        if opt_search_mode in ('filename', 'hash_and_filename'):
            subtitlesSearchList.append(
                {'sublanguageid': currentLanguage, 'query': videoFileName})
        ...
        if ('data' in subtitlesResultList) and (subtitlesResultList['data']):
            ...
            if subtitlesSelected:
                ...
                if opt_output_path and os.path.isdir(os.path.abspath(opt_output_path)):
                    ...
                    subPath = os.path.abspath(
                        opt_output_path) + "/" + subPath.rsplit('/', 1)[1]
                else:
                    # Use the path of the input video
                    subPath = currentVideoPath.rsplit(
                        '.', 1)[0] + '.' + subtitlesResultList['data'][subIndex]['SubFormat']

                # Write language code into the filename?
                if (opt_language_suffix == 'on'):
                    if (opt_language_suffix_size == 2 or opt_language_suffix_size == '2'):
                        subLangId = opt_language_suffix_separator + \
                            subtitlesResultList['data'][subIndex]['ISO639']
                    elif (opt_language_suffix_size == 3 or opt_language_suffix_size == '3'):
                        subLangId = opt_language_suffix_separator + \
                            subtitlesResultList['data'][subIndex]['SubLanguageID']
                    else:
                        subLangId = opt_language_suffix_separator + currentLanguage

                    subPath = subPath.rsplit('.', 1)[
                        0] + subLangId + '.' + subtitlesResultList['data'][subIndex]['SubFormat']
                ...
                if opt_force_utf8:
                    ...
                    if downloadPos > 0:
                        subURL = subURL[:downloadPos+9] + \
                            "subencoding-utf8/" + subURL[downloadPos+9:]

                # Download and unzip the selected subtitles
                if opt_gui == 'gnome':
                    process_subtitlesDownload = subprocess.call("(wget -q -O - " + subURL + " | gunzip > " + subPath + ") 2>&1" + ' | (zenity --auto-close --progress --pulsate --title="Downloading subtitles, please wait..." --text="Downloading <b>' +
                ...
                if process_subtitlesDownload != 0:
                    superPrint("error", "Subtitling error!", "An error occurred while downloading or writing <b>" +
                               subtitlesResultList['data'][subIndex]['LanguageName'] + "</b> subtitles for <b>" + videoTitle + "</b>.")
    ...
    if languageCount_results == 0:
        superPrint("info", "No subtitles available :-(",
